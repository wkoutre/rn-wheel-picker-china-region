...
import staticAreas from "./regionJson";
...
export default class ChinaRegionWheelPicker extends BaseComponent {
  ...
  async componentDidMount() {
    ...
    try {
      area = await webRegionAPI();
    } catch (err) {
      console.log(`Error fetching CN areas:`, err);
      area = staticAreas;
    }
  ...
  renderPicker() {
    ...
    return (
      <View style={styles.overlayStyle}>
        <TouchableWithoutFeedback style={styles.flex1} onPress={onRequestClose}>
        ...
        <View style={styles.pickerContainer}>
          <View style={[styles.navWrap, navStyle]}>
            ...
            >
              <Text
                style={[styles.text, { color: navBtnColor }, leftNavTextStyle]}
              >
                取消
              </Text>
            </TouchableOpacity>
            ...
            {this.props.isShowArea && (
              ...
              >
                {this.state.areas.map((area, index) => {
                  return <Picker.Item value={area} label={area} key={index} />;
                })}
              </Picker>
            )}
          </View>
